...
var fisDeploys = {
  develop: {
    receiver: "http://175.25.18.68:9010/receiver?token=",
    to: ""
  }
};
if (fisDeploys[media]) {
  fis
    ...
    .match(/([a-zA-Z0-9_-]+)\/index.html$/, {
      // Page
      release: "$1.html",
      deploy: [
        fis.plugin("replace", {
          from: /src="\/\/source.ifreetalk.com\/page\/[a-zA-Z0-9/]+\/index.html_aio_[a-zA-z0-9]+.js/,
          to: function($0) {
            return "crossorigin " + $0; // 跨域调试
          }
        }),
...
// 预编译
fis
  .match("{components,public/css}/{**.vue:less,**.less,**.css}", {
    rExt: "css",
  ...
  .match("components/**.vue", {
    ...
    parser: [
      fis.plugin("jdists"),
      fis.plugin("vue-component", { runtimeOnly: true })
    ]
  });
...
// debug 调试
if (!/-w?L/.test(process.argv) && fis.project.currentMedia() === "debug") {
  // 存在监听
  fis.once("release:end", function() {
    openUrl("http://%s:%s/index.php");
  });
}
...
function openUrl() {
  String(package.scripts.debug).replace(/-p\s*(\d+)\s*&&/, function(all, port) {
    setTimeout(function() {
      open(
        util.format("http://%s:%s/indexPage/index.php", debugAddress(), port)
      );
    });
  });
}
